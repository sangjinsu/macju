...
public class MemberController {
    ...
    public ResponseEntity<Map<String, Object>> fetchProfile(@PathVariable Long memberId) {
        ...
        if (member != null) {
            ...
            List<Long> aromas = member.getMemberFondAromaHashTags()
                    .stream()
                    .map(aroma -> aroma.getAromaHashTag().getAromaHashTagId())
                    .collect(Collectors.toList());

            List<Long> flavors = member.getMemberFondFlavorHashTags()
                    .stream()
                    .map(flavor -> flavor.getFlavorHashTag().getFlavorHashTagId())
                    .collect(Collectors.toList());

            result.put("aromas", aromas);
            result.put("flavors", flavors);
